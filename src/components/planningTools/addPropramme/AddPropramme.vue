<template>
  <div>
    <div class="handle">
      <el-select v-model="programmeYear" placeholder="请选择">
      ...
      <el-form :model="formEdit" class="formEdit" ref="formEdit" :rules="rules" label-width="150px">
        ...
        <el-col :span="8">
          <el-form-item label="油耗（L/100km）" prop="yh">
            <el-input
              v-model="formEdit.yh"
              :title="formEdit.yh"
              :disabled="formEdit.tsfa!=='基础方案-新车型'"
              autocomplete="off"
            ></el-input>
          </el-form-item>
        </el-col>
        <el-col :span="8">
          <el-form-item label="纯电续航（km）" prop="cdxh">
            <el-input v-model="formEdit.cdxh" :disabled="formEdit.rylx!=='混动'&&formEdit.rylx!=='纯电动'" :title="formEdit.cdxh" autocomplete="off"></el-input>
          </el-form-item>
        </el-col>
        <el-col :span="8">
...
<script>
import { getYearCxxx } from "@/assets/api/index.js";
import { rulesCopy, rulesCdd,rulesHd } from "@/assets/rules.js";
export default {
  name: "Proprame",
  props: ["programmeName", "programmeYear"],
  data() {
    return {
  ...
  methods: {
    ...
    getYearCxxx() {
      ...
      getYearCxxx(this.programmeYear).then((res) => {
        if (res.ok) {
